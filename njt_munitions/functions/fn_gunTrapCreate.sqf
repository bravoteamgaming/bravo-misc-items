...
private _guns = getArray ((configOf _mine) >> "njt_gunTrapClasses");
...
_container setVariable ["njt_trappedgun_mine", _mine, true];
_mine setVariable ["njt_trappedgun_container", _container, true];

[_mine,
	["Explode", {
		params ["_mine"];
		private _container = _mine getVariable ["njt_trappedgun_container", objNull];
...
[_container,
	["ContainerOpened", {
		params ["_container"];
		private _mine = _container getVariable ["njt_trappedgun_mine", objNull];
		triggerAmmo _mine;
	}]
] remoteExec ["addEventHandler", 0, true];