0 spawn {
	sleep 5;
	if (isNil "bravo_bft_data") then {
		bravo_bft_data = createHashmap;
	};
	...
	while {true} do {
		...
		_markerWave = [];
		if ((count bravo_bft_data) < 1) then {sleep 5; continue};
		...
		{
			private _data = bravo_bft_data get _x;
			_data params ["_type", "_colour", "_name"];
			
			private _marker = createMarkerLocal ["bravo_bft_marker_" + _x, objectFromNetId _x];
			_marker setMarkerShapeLocal "ICON";
			_marker setMarkerTypeLocal _type;
			_marker setMarkerColorLocal _colour;
			_marker setMarkerTextLocal _name;
			_markerWave pushBack _marker;
		} forEach keys bravo_bft_data;
		sleep 5;
	};
};