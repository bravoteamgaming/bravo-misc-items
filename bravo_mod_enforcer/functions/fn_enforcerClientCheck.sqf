...
0 spawn {
	...
	/*
	if (((_requiredModsStr select [0,1]) != "[") or ((_requiredModsStr select [) != "]")) exitWith {
		"[BRAVO] WARNING: MOD ENFORCER IS NOT CORRECTLY CONFIGURED AND WILL NOT RUN. CHECK CBA SETTINGS." remoteExec ["systemChat"];
	};
	*/
	...
	private _loadedMods = ("true" configClasses (configFile >> "CfgPatches")) apply {toLowerANSI configName _x};

	private _missingMods = _requiredMods - _loadedMods;

	if (_missingMods isNotEqualTo []) then {
		...
		private _formatText = ["[Bravo] User %1 is missing required mods and has been kicked. %2 - %3", profileName, _phrase, str _missingMods];
