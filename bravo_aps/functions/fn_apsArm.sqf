...
if (_args#0) then {
	...
	[_text,2,0] remoteExec ["bravo_fnc_apsLocalWarning", (crew _target) select {isPlayer _x}];
	[["beep",2]] remoteExec ["playSound",crew _target];

	_target spawn {
		sleep 1;
		[_this] remoteExec ["bravo_fnc_apsLoop",0,true];
	};
} else {
	params ["_target", "_caller", "_args"];
	_target setVariable ["bravo_var_apsEnabled",false,true];
	["APS DISARM",2,0] remoteExec ["bravo_fnc_apsLocalWarning", (crew _target) select {isPlayer _x}];
	[["beep",2]] remoteExec ["playSound",crew _target];
};