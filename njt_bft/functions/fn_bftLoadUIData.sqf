waitUntil {
	(!isNull (uiNamespace getVariable ["njt_bft_currentObject", objNull])) or !(uiNamespace getVariable ['njt_bft_displayOpen',false]);
};
if !(uiNamespace setVariable ['njt_bft_displayOpen',false]) exitWith{};
private _object = uiNamespace getVariable ["njt_bft_currentObject", objNull];

private _data = njt_bft_data getOrDefault [netId _object, ["",[0,0,0],""]];
_data params ["_currentClass", "_currentColour", "_currentName"];

private _objclasscontrol = uiNamespace getVariable ["njt_bft_classnameBox", controlNull];
_objclasscontrol ctrlSetText typeOf _object;

private _mcontrol = uiNamespace getVariable ["njt_bft_markerListBox", controlNull];
...
private _ccontrol = uiNamespace getVariable ["njt_bft_colourListBox", controlNull];
...
private _ncontrol = uiNamespace getVariable ["njt_bft_dataNameBox", controlNull];
